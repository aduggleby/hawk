...
- The footer in `_Layout.cshtml` displays the version (e.g. `v0.9.18`) alongside a link to the GitHub repo and author.
...
- In Development, an expandable `<details>` section shows the full stack trace and inner exception chain.
- Uses `IExceptionHandlerPathFeature` to extract exception details when available.
