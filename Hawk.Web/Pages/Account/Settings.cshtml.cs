// <file>
// <summary>
// Account settings: alert recipient override + crawler (User-Agent) override.
// </summary>
// </file>
...
public sealed class SettingsModel(ApplicationDbContext db, UserManager<IdentityUser> userManager, IConfiguration config) : PageModel
{
    ...
    public async Task<IActionResult> OnPostCrawlerAsync(CancellationToken cancellationToken)
    {
        ...
        // Validate UA only for length; it can be either a preset key or a full UA string.
        if (!ModelState.IsValid)
            return Page();
        ...
        // Store the raw value (preset key or UA string). Resolution happens at runtime.
        if (existing is null)
        {
